...
main()
{
    ...
    /*
            ...
            [](int count)
        {
            if (count > 100) return crow::response(crow::status::BAD_REQUEST);
    ...
    */
    ...
    CROW_ROUTE(app, "/").name("hello")([] { return "Hello World check #2!"; });
    ...
    (
        []
        {
            return crow::json::wvalue({
                {"first",   "Hello world!"                   }, /* stores a char const* hence a
  json::type::String */
                {"second",  std::string("How are you today?")}, /* stores a
  std::string hence a   json::type::String. */
                {"third",
                 std::int64_t(
                     54)                                     }, /* stores a 64-bit int hence a std::int64_t. */
                {"fourth",  std::uint64_t(54)                }, /* stores a 64-bit unsigned int
  hence a std::uint64_t. */
                {"fifth",   54                               }, /* stores an int (as 54 is an int literal) hence
  a std::int64_t. */
                {"sixth",   54u                              }, /* stores an unsigned int (as 54u is a unsigned
  int literal) hence a std::uint64_t. */
                {"seventh", 2.f                              }, /* stores a float (as 2.f is a float literal)
  hence a double. */
                {"eighth",  2.                               }, /* stores a double (as 2. is a double literal)
  hence a double. */
                {"ninth",
                 nullptr                                     }, /* stores a std::nullptr hence json::type::Null . */
    ...
    // To see it in action enter {ip}:18080/hello/{integer_between -2^32 and
    // 100} and you should receive {integer_between -2^31 and 100} bottles of
    // beer!
    ...
    // Compile error with message "Handler type is mismatched with URL
    // paramters"
    ...
    (
        [](const crow::request& req)
        {
            ...
            os << "The key 'foo' was "
               << (req.url_params.get("foo") == nullptr ? "not " : "")
               << "found.\n";
            ...
            {
                // double countD =
                // crow::utility::lexical_cast<double>(req.url_params.get("pew"));
                // os << "The value of 'pew' is " << countD << '\n';
            }

            // To get a list from the request
            // You have to submit something like '/params?count[]=a&count[]=b'
            // to have a list with two values (a and b)
            ...
            // You have to submit something like
            // '/params?mydict[a]=b&mydict[abcd]=42' to have a list of pairs
            // ((a, b) and (abcd, 42))
